...
export default function Dashboard() {
  ...
  async function loadAll() {
    const [{ data: s }, { data: c }, { data: t }, { data: cb }] = await Promise.all([
      ...
      supabase.from('campaigns').select('*').eq('is_active', true).is('parent_campaign_id', null).order('created_at', { ascending: false }),
  ...
  async function deleteCampaign(e: React.MouseEvent, campaignId: string, campaignName: string) {
    e.preventDefault()
    e.stopPropagation()
    if (!confirm(`Delete "${campaignName}"? This will also delete all sub-campaigns and leads inside it. This cannot be undone.`)) return
    // Delete all leads in sub-campaigns
    const { data: subs } = await supabase.from('campaigns').select('id').eq('parent_campaign_id', campaignId)
    if (subs && subs.length > 0) {
      const subIds = subs.map((s: any) => s.id)
      await supabase.from('leads').delete().in('campaign_id', subIds)
      await supabase.from('campaigns').delete().in('id', subIds)
    }
